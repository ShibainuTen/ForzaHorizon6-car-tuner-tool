...
// 詳細なチューニング項目（ツールが提案する全項目）
interface DetailedTuningProposal {
  id: number; // 保存された仕様ID
  notes: string; // メモ（例：初期提案、進入アンダー対策など）
  tirePressure: { front: number; rear: number }; // 空気圧bar
  gearing: { final: number; gears: number[] }; // ファイナル, 段階
  alignment: { 
    camber: { front: number; rear: number }; 
    toe: { front: number; rear: number }; 
    caster: number 
  }; 
  antiRollBar: { front: number; rear: number }; // スタビライザー
  springs: { 
  ...
  aero: { downforce: { front: number; rear: number } }; // エアロダウンフォースkg
  brake: { balance: number; pressure: number }; // ブレーキバランス%, 圧力%
  differential: { accel: number; decel: number }; // デフ加速%, 減速%
}
...
const calculateInitialSetup = (weight: number, driveType: string, category: string): DetailedTuningProposal => {
  ...
  return {
    ...
    antiRollBar: { front: baseFrontSpring * 0.1, rear: baseRearSpring * 0.1 }, // スプリングの10%
    springs: { 
...
// フィードバックに基づいて「セッティングをピンポイント修正」する
const updateSetupBasedOnFeedback = (current: DetailedTuningProposal, feedback: TestDriveFeedback): DetailedTuningProposal => {
  const next = JSON.parse(JSON.stringify(current)) as DetailedTuningProposal; // ディープコピー
  next.id = current.id + 1;
  let notes: string[] = [];

  // --- ハンドリング関連（アンダー・オーバー）の修正ロジック ---
  if (feedback.handling === 'EntryUnder') {
    next.antiRollBar.front *= 0.9; // フロントを柔らかく
    next.damping.bump.front *= 0.9;
    next.aero.downforce.front += 5; // フロントの羽を立てる
    next.notes = `${current.notes} -> 進入アンダー対策`;
  } else if (feedback.handling === 'ExitOver') {
    next.differential.accel *= 0.9; // リアデフ加速を柔らかく
    next.antiRollBar.rear *= 0.9; // リアを柔らかく
    next.springs.spring.rear *= 0.95;
    next.aero.downforce.rear += 5; // リアの羽を立てる
  ...
  if (feedback.speed === 'MoreTopSpeed') {
    next.gearing.final *= 0.95; // ファイナルを小さく
    next.aero.downforce.front *= 0.95; // ダウンフォースを少し削る
    next.aero.downforce.rear *= 0.95;
    notes.push('最高速重視');
  } else if (feedback.speed === 'BadAccel') {
    next.gearing.final += 0.1; // ファイナルを大きく
    notes.push('加速重視');
  }
...
export default function App() {
  ...
  const [carName, setCarName] = useState("NISSAN GT-R '17");
  const [piClass, setPiClass] = useState('A');
  ...
  const [proposalHistory, setProposalHistory] = useState<DetailedTuningProposal[]>([]);
  ...
  const saveLog = () => {
    ...
    const newLog: TuningLog = {
      ...
      lapTime: lapTime || '--:--.--',
      note: note || '特記事項なし',
      proposal: JSON.parse(JSON.stringify(currentProposal)) // 保存時の仕様を保持
    };
    setLogs([newLog, ...logs]);
    setRunCount(runCount + 1);
  };

  // 📦 Rendering Helper for Proposals
    ...
    <div style={{ padding: '15px', maxWidth: '600px', margin: '0 auto', fontFamily: 'sans-serif', backgroundColor: '#1e293b', color: '#fff', borderRadius: '10px' }}>
      ...
      <div style={{ backgroundColor: '#334155', padding: '10px', borderRadius: '6px', marginBottom: '15px', fontSize: '12px' }}>
        <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
          <div style={{ flex: 1 }}>重量: <input type="number" value={weight} onChange={(e) => setWeight(Number(e.target.value))} style={{ width: '50px', padding: '3px', borderRadius: '3px', backgroundColor: '#1e293b', color: '#fff', border: 'none' }} /> kg</div>
          <div style={{ flex: 1 }}>馬力: <input type="number" value={hp} onChange={(e) => setHp(Number(e.target.value))} style={{ width: '50px', padding: '3px', borderRadius: '3px', backgroundColor: '#1e293b', color: '#fff', border: 'none' }} /> hp</div>
            ...
            {logs.map((log) => (
              <div key={log.id} style={{ backgroundColor: '#334155', padding: '8px', borderRadius: '5px', fontSize: '11px' }}>
                ...
                <details>
                  ...
                  <pre style={{ fontSize: '10px', color: '#cbd5e1', padding: '5px', backgroundColor: '#1e293b', borderRadius: '4px', marginTop: '3px' }}>
...
デフ accel: ${log. differential.accel}%, decel: ${log.differential.decel}%`}
                  </pre>
                </details>
